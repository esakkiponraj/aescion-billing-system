...
export const InvoiceFormPage: React.FC = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'CASH' | 'UPI' | 'CARD' | 'BANK_TRANSFER' | 'CREDIT'>('CASH');
  ...
  // Sync paidAmount with grandTotal when not on credit
  useEffect(() => {
    if (paymentMethod !== 'CREDIT') {
      setPaidAmount(calculated.grandTotal);
    }
  }, [paymentMethod, calculated.grandTotal]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validItems.length === 0) {
      alert('Please add at least one valid line item with a description and quantity.');
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        paymentMethod,
        paidAmount: paymentMethod === 'CREDIT' ? Number(paidAmount) : calculated.grandTotal,
      ...
      const invoice = await apiRequest('/finance/invoices', {
        method: 'POST',
    ...
    <div className="p-3 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900">Create Tax Invoice</h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Generate a new invoice. Inventory stock will be deducted and payment receipts generated.
            </p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-2xs grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs font-semibold text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-brand-500/20"
            >
              <option value="CASH">Cash (Full Settlement)</option>
              <option value="UPI">UPI / Digital Payment</option>
              <option value="CARD">Credit / Debit Card</option>
              <option value="BANK_TRANSFER">Bank Transfer (NEFT/RTGS)</option>
              <option value="CREDIT">Credit (Partial or Zero Advance)</option>
            </select>
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-2xs overflow-hidden">
          <div className="p-4 bg-slate-50/70 border-b border-slate-200 flex items-center justify-between">
            <div>
              <h2 className="text-xs font-bold uppercase tracking-wider text-slate-700">Invoice Items</h2>
              <p className="text-[11px] text-slate-400 mt-0.5">Select products or enter custom line items</p>
            </div>
            <button
              type="button"
              onClick={addItemRow}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            {paymentMethod === 'CREDIT' && (
              <div className="bg-amber-50 p-4 rounded-xl border border-amber-200 space-y-2">
                <label className="block text-xs font-bold text-amber-900">Initial Paid Amount (₹)</label>
                <input
                  type="number"
                  min="0"
                  max={calculated.grandTotal}
                  step="0.01"
                  value={paidAmount}
                ...
                <p className="text-[11px] text-amber-700">
                  Remaining balance of ₹{Math.max(0, calculated.grandTotal - paidAmount).toFixed(2)} will be tracked in Accounts Receivable.
                </p>
              </div>
            )}
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-2.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/20"
                placeholder="Optional customer or delivery notes..."
              />
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-2xs space-y-3">
            ...
            <div className="border-t border-slate-100 pt-2 flex justify-between items-center text-xs text-emerald-700 font-semibold">
              <span>Paid Amount:</span>
              <span className="font-mono">₹{(paymentMethod === 'CREDIT' ? paidAmount : calculated.grandTotal).toFixed(2)}</span>
            </div>
          </div>
        </div>
      </form>
